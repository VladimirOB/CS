...
namespace SnakeGame
{
    public class Snake
    {
        ...
        {
            Clear();
            Body.Enqueue(new Pixel(Head.X, Head.Y, _bodyColor)); // добавляем в тело новый пиксель на место головы

            if(!eat) // если не ест
            Body.Dequeue(); // убираем последний пиксель

            Head = direction switch // двигаем голову взавизимости от направления
            {
